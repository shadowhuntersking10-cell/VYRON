...
class BroadcastBody(BaseModel):
    title_key: str = "order_created"
    body_key: str = "order_created"
...
def health(db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    checks["WEBHOOKS"] = {
        "status": "Configured" if settings.payment_configured() else "Not configured",
        "detail": "/api/webhooks/payments, /api/webhooks/payerpin",
    }
    checks["STORAGE"] = {
        "status": "Configured",
        "detail": "database-driven assets",
    }
...
def refund_order(
    ...
    """Initiate a refund via the payment provider (separated from fulfillment)."""
    order = db.get(Order, order_id)
    if order is None:
        raise HTTPException(status_code=404, detail="order_not_found")
    ...
    if provider is None or payment is None or not payment.provider_payment_id:
    ...
    try:
        ...
        # FastAPI runs in an event loop already — use the async path below.
    except Exception:
        result = None
    raise HTTPException(status_code=502, detail="use_async_refund")  # replaced below
